...
INSTALLMETHODS   = ['Sempre Pergunte', 'Recarregar Perfil', 'Forca Fechamento do Kodi']
...
def viewBuild(name):
	...
	if wiz.checkBuild(name, 'version') == False: 
		addFile('erro ao ler o arquivo HTM.', '', themeit=THEME3)
...
def apkScraper(name=""):
	if name == 'kodi':
		...
		addFile("Oficial Kodi Apk\'s", themeit=THEME1)
		rc = False
		...
		addFile("Oficial SPMC Apk\'s", themeit=THEME1)
...
def apkMenu(url=None):
	if url == None:
		addDir ('Oficial Kodi Apk\'s', 'apkscrape', 'kodi', icon=ICONAPK, themeit=THEME1)
		addDir ('Oficial SPMC Apk\'s', 'apkscrape', 'spmc', icon=ICONAPK, themeit=THEME1)
		if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
	if not APKFILE == 'http://':
		if url == None:
		...
		return
	else: wiz.log("[APK Menu] Nenhuma lista de APK Adicionada.")
	setView('files', 'viewType')
					...
					if plugin.lower() == 'section':
						...
						try:
							...
							if os.path.exists(add):
								name   = "[COLOR green][Instalado][/COLOR] %s" % name
						except:
							pass
						x += 1
...
def writeAdvanced(name, url):
	...
	if ADVANCEDWORKING == True:
		...
		else: choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Gostaria de baixar e instalar a BUILD abaixo? [COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, name), yeslabel="[B][COLOR green]Instalar[/COLOR][/B]", nolabel="[B][COLOR red]Cancelar[/COLOR][/B]")
...
def buildWizard(name, type, theme=None, over=False):
	...
	if type == 'gui':
		if name == BUILDNAME:
			...
			yes = DIALOG.yesno("%s - [COLOR red]ATENCAO!![/COLOR]" % ADDONTITLE, "[COLOR %s][COLOR %s]%s[/COLOR] a compilacao da comunidade NAO esta Instalada Atualmente." % (COLOR2, COLOR1, name), "Voce Gostaria de Aplicar o guiFix mesmo assim?.[/COLOR]", nolabel='[B][COLOR red]Nao, Cancelar[/COLOR][/B]',yeslabel='[B][COLOR green]Aplicar![/COLOR][/B]')
		if yes:
		...
		if yes_pressed:
			...
			if int(float(percent)) > 0:
				...
				else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Voce Gostaria de? [COLOR %s]Encerrar o[/COLOR] kodi ou [COLOR %s]Recarregar Perfil[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR red]Recarregar Perfil[/COLOR][/B]", nolabel="[B][COLOR green]Encerrar Kodi[/COLOR][/B]")
				if todo == 1: wiz.reloadFix()
			...
			if not themefile == 'http://' and wiz.workingURL(themefile) == True:
				...
				if len(themelist) > 0:
					if DIALOG.yesno(ADDONTITLE, "[COLOR %s]A Build [COLOR %s]%s[/COLOR] Vem Com [COLOR %s]%s[/COLOR] Diferentes Temas" % (COLOR2, COLOR1, name, COLOR1, len(themelist)), "Gostaria de Instalar um Agora?[/COLOR]", yeslabel="[B][COLOR green]Instalar Temas[/COLOR][/B]", nolabel="[B][COLOR red]Cancelar Temas[/COLOR][/B]"):
						...
						else: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Instacao do Tema: Cancelado![/COLOR]' % COLOR2); return
...
def thirdPartyInstall(name, url):
	...
	else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Voce Gostaria de? [COLOR %s]Encerrar o Kodi[/COLOR] kodi or [COLOR %s]Recarregar Perfil[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR green]Recarregar Perfil[/COLOR][/B]", nolabel="[B][COLOR red]Encerrar o Kodi[/COLOR][/B]")
	if todo == 1: wiz.reloadFix()
	else: wiz.killxbmc(True)
...
def apkInstaller(apk, url):
	...
	if wiz.platform() == 'android':
		yes = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Gostaria de Baixar e Instalar?" % COLOR2, "[COLOR %s]%s[/COLOR]" % (COLOR1, apk), yeslabel="[B][COLOR green]Baixar[/COLOR][/B]", nolabel="[B][COLOR red]Cancelar[/COLOR][/B]")
		...
		if os.path.exists(trakt):
			...
			for item in files:
				...
				if os.path.exists(old):
					if not DIALOG.yesno(ADDONTITLE, "[COLOR %s]Voce Gostaria de Substituir o Arquivo Atual? [COLOR %s]%s[/COLOR] file?" % (COLOR2, COLOR1, item), yeslabel="[B][COLOR green]Substituir[/COLOR][/B]", nolabel="[B][COLOR red]Nao, Cancelar![/COLOR][/B]"): continue
					else: os.remove(old)
...
def freshStart(install=None, over=False):
	...
	elif install == 'restore': yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Voce Deseja Restaurar Seu" % COLOR2, "Kodi Para as Configuracoes Padroes", "Antes de Instalar o Backup Local?[/COLOR]", nolabel='[B][COLOR red]Nao, Cancelar[/COLOR][/B]', yeslabel='[B][COLOR green]Continue[/COLOR][/B]')
	elif install: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Voce Deseja Restaurar Seu" % COLOR2, "Kodi Para as Configuracoes Padroes", "Antes de Instalar? [COLOR %s]%s[/COLOR]?" % (COLOR1, install), nolabel='[B][COLOR red]Nao, Cancelar[/COLOR][/B]', yeslabel='[B][COLOR green]Continue[/COLOR][/B]')
	else: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Voce Deseja Restaurar Seu" % COLOR2, "Kodi Para as Configuracoes Padroes?[/COLOR]", nolabel='[B][COLOR red]Nao, Cancelar[/COLOR][/B]', yeslabel='[B][COLOR green]Continue[/COLOR][/B]')
	if yes_pressed:
		if not wiz.currSkin() in ['skin.confluence', 'skin.estuary']:
			skin = 'skin.confluence' if KODIV < 17 else 'skin.estuary'
...
####THANKS GUYS @ NaN #######
def clearCache():
	if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Voce Gostaria de Limpar o Cache?[/COLOR]' % COLOR2, nolabel='[B][COLOR red]Nao, Cancelar[/COLOR][/B]', yeslabel='[B][COLOR green]Limpe o Cache[/COLOR][/B]'):
		wiz.clearCache()
		#clearThumb()

def totalClean():
	if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Voce Gostaria de Limpar o Cache, Pacotes e Miniaturas?[/COLOR]' % COLOR2, nolabel='[B][COLOR red]Nao, Cancelar Processo[/COLOR][/B]',yeslabel='[B][COLOR green]Limpe Tudo![/COLOR][/B]'):
		wiz.clearCache()
...
elif mode=='clearwizlog'    : f = open(WIZLOG, 'w'); f.close(); wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Registo do Assistente Apagado![/COLOR]" % COLOR2)
