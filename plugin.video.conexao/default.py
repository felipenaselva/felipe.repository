...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(Conexão TV,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(Conexão TV,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")	
				
def CHIndex():
    addon_log("CHIndex")
    addDir('[COLOR yellow][B]Séries Online HD [/B][/COLOR][COLOR red](ADDON GRÁTIS)[/COLOR]','plugin.video.seriesonlineh1',100,series,'','','','','')
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(Conexão TV,New source added.,5000,"+icon+")")
        if not url is None:
                ...
                if len(item('link')) >0:
                    ...
                    for i in item('torrent'):
                        if not i.string == None:
                            ...
                            key = 1 #Aqui você troca para quasar 1 ou 2 para pulsar
