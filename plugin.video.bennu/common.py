...
## ################################################## ##
OverlayBorder=artp(''); 
OverlayBackground=artp('note'); 
hubLogo=artp(''); 
DefaultNoteImage=artp(''); 
DefaultSplitter="|||"; 
...
class MyWindow(xbmcgui.WindowDialog): #xbmcgui.Window): ##xbmcgui.Window
    ...
    def __init__(self,noteType='t',noteMessage='',noteImage='',L=625,T=10,W=650,H=700,Font='font10',TxtColor='FFA05000'):
        ...
        self.BG=xbmcgui.ControlImage(L,T,W,H,self.background,aspectRatio=0,colorDiffuse='none'); 
        ...
        ## ### ## 
        focus=artp('button-focus_grey'); nofocus=artp('button-focus_lightblue');
        w1=-10; h1=-10; w2=900; h2=800; spacing1=10; 
        l2=L+W-spacing1-w2; t2=T+H-h2-spacing1; 
        l1=L+W-spacing1-w2-spacing1-w1; t1=T+H-h1-spacing1; 
        self.buttonDismiss=xbmcgui.ControlButton(l1,t1,w1,h1,"",textColor="0xFF000000",focusedColor="0xFF000000",alignment=2,focusTexture=focus,noFocusTexture=nofocus); 
        self.buttonRemindMe=xbmcgui.ControlButton(l2,t2,w2,h2,"",textColor="0xFF000000",focusedColor="0xFF000000",alignment=2,focusTexture=focus,noFocusTexture=nofocus); 
        #self.OlaySplash
        ...
        for z in [self.BG,self.ImgMessage,self.TxtMessage,self.iLogo,self.buttonRemindMe,self.buttonDismiss]: 
            ...
            z.setAnimations([('WindowOpen','effect=slide delay=0 time=2000 start=2000,0 end=0'),('WindowClose','effect=slide delay=0 time=2000 start=0 end=2000,0'+str(0-(H+T+10)))]); 
        ## ### ## 
...
def FetchNews():
    ...
    if os.path.isfile(info_location)==True: 
        ...
        try: 
            ...
            html=cache.get(client.request, 3, info_location2)
...
def CheckNews(TypeOfMessage,NewImage,NewMessage,DoFromService=True):
    if (len(NewImage) > 0) or (len(NewMessage) > 0):
        ...
        if (tfalse(SettingG("notifications-on-startup"))==False) or (DoFromService==False):
            #mes_file = os.path.join(xbmc.translatePath(os.path.join('special://profile/addon_data/', 'plugin.video.bennu')), 'notice.txt')
            #if not os.path.exists(mes_file): 
            #    with open(mes_file, 'w') as f: pass
            #with open(mes_file) as f: old_mes = f.read()
            #if NewMessage != old_mes:
            #    with open(mes_file, 'w') as f: f.write(NewMessage)
