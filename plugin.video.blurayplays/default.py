...
addon = xbmcaddon.Addon(id='plugin.video.blurayplays')
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.live.blurayplays Lists-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(blurayplays,We failed with error code - "+str(e.code)+",10000,"+icon+")")
...
play = os.path.join(xbmc.translatePath("special://home/addons/plugin.video.blurayplays/playlistv.xml").decode("utf-8"))
...
def SKindex():
    ...
    getData(off,'')
    msg2 = "https://pastebin.com/raw/PCVVd1t1"				
    msg = msg2 
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(blurayplays,New source added.,5000,"+icon+")")
        if not url is None:
...
#hakamac code
    ...
    progress.create('blurayplays Searching Please wait',' ')
...
def urlsolver(url):
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(blurayplays,Please enable Update Commonresolvers to Play in Settings. - ,10000)")

    resolved=genesisresolvers.get(url).result
    if url == resolved or resolved is None:
        #import
        xbmc.executebuiltin("XBMC.Notification(blurayplays,Using Urlresolver module.. - ,5000)")
        import urlresolver
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('blurayplays','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('blurayplays', 'Do you want to add this file as a source?')
        if ret:
...
def search(site_name,search_term=None):
    ...
    if 'youtube' in site_name:
        ...
        if res == 'True':
            ...
            xbmc.executebuiltin("XBMC.Notification(blurayplays,No IMDB match found ,7000,"+icon+")")
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie=""):
        ...
        if regexs: 
            ...
            if addon.getSetting('dlaudioonly') == 'true':
                ...
                url = 'plugin://plugin.video.f4mTester/?url=' +url+ '&amp;streamtype=HLS&amp;name=blurayplays'
                mode = '12'
        elif '.ts' in url:
                url = 'plugin://plugin.video.f4mTester/?url=' +url+ '&amp;streamtype=TSDOWNLOADER&amp;name=blurayplays'
                mode = '12'
...
if mode==None:
    ...
    if url:
        ...
        xbmc.executebuiltin("XBMC.Notification(blurayplays ,Failed to extract regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification(blurayplays,Please [COLOR yellow]install the Youtube Addon[/COLOR] module ,10000,"")")
