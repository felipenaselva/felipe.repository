...
from elementum.util import toUtf8, notify, getLocalizedString, getLocalizedLabel, system_information
    ...
    def AddonFailure(self, addonId):
        ...
        if self._failures[addonId] > int(ADDON.getSetting("provider_allowed_failures")):
            try:
                ...
                urllib2.urlopen("%s/provider/%s/disable" % (ELEMENTUMD_HOST, addonId))
            except:
...
def server_thread():
    try:
        s = bjsonrpc.createserver(port=JSONRPC_EXT_PORT, handler_factory=ElementumRPCServer)
