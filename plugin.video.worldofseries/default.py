...
FANART2 = os.path.join(home, 'fanart.gif')
...
hash  = "6ZlR4pVQqV0L3Fmcv02bj5ibpJWZ0NXYw9yL6MHc0RHa"
tam = len(hash)
calc = hash[::-1]
CHBase = base64.decodestring(calc)
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.boxserieshd-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(boxserieshdPlay,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(boxserieshd,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
