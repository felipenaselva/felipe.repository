...
addon = xbmcaddon.Addon('plugin.video.AK-47')
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.AK-47-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(AK - 47,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(AK - 47,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(AK - 47,New source added.,5000,"+icon+")")
        if not url is None:
...
def SportsDevil():
    print "SportsDevil"
    url = "http://repo.run/zip/plugin.video.SportsDevil/plugin.video.SportsDevil-2.zip"
    ...
    xbmcgui.Dialog().ok('SportsDevil', 'Successfully installed!')

def f4mTester():
    print "f4mTester"
    url = "http://repo.run/zip/plugin.video.f4mTester/plugin.video.f4mTester-2.2.5.zip"
    addonsDir = xbmc.translatePath(os.path.join('special://home', 'addons')).decode("utf-8")
    packageFile = os.path.join(addonsDir, 'packages', 'f4m.zip')
    ...
    xbmc.executebuiltin("UpdateLocalAddons")
    xbmc.executebuiltin("UpdateAddonRepos")
    xbmcgui.Dialog().ok('f4mTester', 'Successfully installed!')

def ZemTV():
    print "ZemTV-shani"
    url = "http://repo.run/zip/plugin.video.ZemTV-shani/plugin.video.ZemTV-shani-4.9.6.zip"
    addonsDir = xbmc.translatePath(os.path.join('special://home', 'addons')).decode("utf-8")
    packageFile = os.path.join(addonsDir, 'packages', 'ZemTV.zip')
    ...
    xbmc.executebuiltin("UpdateLocalAddons")
    xbmc.executebuiltin("UpdateAddonRepos")
    xbmcgui.Dialog().ok('ZemTV', 'Successfully installed!')

def LiveStreamPro():
    print "LiveStreamPro"
    url = "http://repo.run/zip/plugin.video.live.streamspro/plugin.video.live.streamspro-2.4.4.zip"
    addonsDir = xbmc.translatePath(os.path.join('special://home', 'addons')).decode("utf-8")
    packageFile = os.path.join(addonsDir, 'packages', 'LSP.zip')
    ...
    xbmc.executebuiltin("UpdateLocalAddons")
    xbmc.executebuiltin("UpdateAddonRepos")
    xbmcgui.Dialog().ok('LiveStreamPro', 'Successfully installed!')

def XtreamCodes():
    print "XtreamCodes"
    url = "http://repo.run/zip/plugin.video.xtream-codes/plugin.video.xtream-codes-2.0.zip"
    addonsDir = xbmc.translatePath(os.path.join('special://home', 'addons')).decode("utf-8")
    packageFile = os.path.join(addonsDir, 'packages', 'XTC.zip')
    ...
    xbmc.executebuiltin("UpdateLocalAddons")
    xbmc.executebuiltin("UpdateAddonRepos")
    xbmcgui.Dialog().ok('XtreamCodes', 'Successfully installed!')

def Addic7edSubstitles():
    print "Addic7edSubstitles"
    url = "http://repo.run/zip/service.subtitles.addic7ed/service.subtitles.addic7ed-1.0.0.zip"
    addonsDir = xbmc.translatePath(os.path.join('special://home', 'addons')).decode("utf-8")
    packageFile = os.path.join(addonsDir, 'packages', 'addic7ed.zip')
    ...
    xbmc.executebuiltin("UpdateLocalAddons")
    xbmc.executebuiltin("UpdateAddonRepos")
    xbmcgui.Dialog().ok('Addic7edSubstitles', 'Successfully installed!')

def A47Skin():
    print "A47Skin"
    url = "http://repo.run/zip/skin.aeon.nox.5/skin.aeon.nox.5-5.8.6.zip"
    addonsDir = xbmc.translatePath(os.path.join('special://home', 'addons')).decode("utf-8")
    packageFile = os.path.join(addonsDir, 'packages', 'aeon.zip')
    ...
    xbmc.executebuiltin("UpdateLocalAddons")
    xbmc.executebuiltin("UpdateAddonRepos")
    xbmcgui.Dialog().ok('A-47 Moded Skin', 'Successfully installed!')
...
def getSoup(url,data=None):
        ...
        if url.startswith('http://') or url.startswith('https://'):
            ...
            if '//fuckoff/' in url:
                enckey=url.split('//fuckoff/')[1].split('//')[0]
                rp='//fuckoff/%s//'%enckey
                url=url.replace(rp,"")
...
def urlsolver(url):
    ...
    if host:
        ...
        xbmc.executebuiltin("XBMC.Notification(AK - 47,Urlresolver donot support this domain. - ,5000)")
        resolver=url
    return resolver
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('AK - 47','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('AK - 47', 'Do you want to add this file as a source?')
        if ret:
...
#            print 'parentalblockedpin',parentalblockedpin
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('Remove from AK - 47 Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
            ...
            if not name in FAV:
                contextMenu.append(('Add to AK - 47 Favorites','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
#                print 'setting as folder in link'
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remove from AK - 47 Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('Add to AK - 47 Favorites','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
...
#            ln+='</item>'
        ...
        if url:
            ...
            xbmc.executebuiltin("XBMC.Notification(AK - 47,Failed to extract regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification(AK - 47,Please [COLOR yellow]install Youtube-dl[/COLOR] module ,10000,"")")
    ...
    if not (keyboard.isConfirmed() == False):
        ...
        if newStr==parentalblockedpin:
            ...
            xbmc.executebuiltin("XBMC.Notification(AK - 47,Parental Block Disabled,5000,"+icon+")")
        else:
            xbmc.executebuiltin("XBMC.Notification(AK - 47,Wrong Pin??,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
elif mode==56:
    addon_log("disable lock")
    addon.setSetting('parentalblocked', "true")
    xbmc.executebuiltin("XBMC.Notification(AK - 47,Parental block enabled,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
